...
class Argument {
    ...
    async obtain(msg, value, promptLimit = Infinity) {
        ...
        while (!valid || typeof valid === 'string') {
            ...
            */
            prompts.push(await msg.reply(stripIndents`
                ${empty ? this.prompt : valid ? valid : `無効な${this.label}を入力しました、正しく入力してください。`}
                ${oneLine`
                    \`cancel\`と入力するとコマンドをキャンセルできます。
                    ${wait ? `${this.wait}秒後に自動でキャンセルされます。` : ''}
                `}
            `));
            ...
            while (!valid || typeof valid === 'string') {
                ...
                if (value) {
                    ...
                    */
                    prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							無効な${this.label}を入力しました、
							"${escaped.length < 1850 ? escaped : '[メッセージが長すぎます]'}".
							正しく入力してください.
						`}
						${oneLine`
                            \`cancel\`と入力するとコマンドをキャンセルできます。
                    ...
                    */
                    prompts.push(await msg.reply(stripIndents`
						...
						${oneLine`
                            \`cancel\`と入力するとコマンドをキャンセルできます。
                            ${wait ? `応答がない場合、${this.wait}秒後に自動でキャンセルされます。` : ''}
                        `}
					`));
                }
